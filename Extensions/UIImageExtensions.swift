...
}// block of code adds a new extension to UI image to confom to transferable, Transferable
// representation descirbes how to transfer an item and when the imported UTType is an image it will import the image as data and construct a UI image from the data.
// Lastly it will create the UI image and if not display an error for the image 
